...
from factor_engine.factories.fundamental.pipeline_fund import pack_code_for_remote
from factor_engine.factories.fundamental_single_metric.llm_chain_fund_single import (
    run_three_stages_with_memory,
)
...
def run_single_metric_pipeline(
    config,
    data_folders,
    useful_fields,
    field_governance,
    ...
    """
    Single-metric financial-factor generation pipeline.

    The data bridge, operator header, remote submission, and optional local execution
    reuse the main fundamental factory. The LLM prompt/chain is the single-metric one.
    """
    if snapshot_days is None:
        snapshot_days = generate_snapshot_calendar(2016, 2026)
    remote_url = get_platform_url()

    print(f"[Init] Fundamental single-metric factory booting with data: {data_folders}")
    ...
    for round_i in range(max_rounds):
        print(f"\n[Single-Metric Round {round_i + 1}/{max_rounds}] Starting...")
        ...
        print("[Brain] Generating single-metric factor logic and code...")
        ...
        if not code_text or not design_text:
            print("[Warn] Single-metric generation returned empty output. Skipping round.")
            continue

        print("[Remote] Submitting single-metric fundamental batch to platform...")
                ...
                try:
                    ...
                    for factor_name, factor_data in results.items():
                        ...
                        if isinstance(factor_data, pd.DataFrame):
                            ...
                            saved_count += 1
                    print(
                        f"[Local] Saved {saved_count} single-metric factors to {local_save_dir}"
                    )
                except Exception as exc:
                    print(f"[Local] Single-metric execution failed: {exc}")
            else:
